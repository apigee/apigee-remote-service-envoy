...
def deploy(logger):
  ...
  cmd = [os.getenv("CLI_DIR", ".")+"/apigee-remote-service-cli", "samples", "create",
          "-c", "config.yaml", "--out", "opdk-istio", "-t", istio_version, "-f"]
  ...
  deployment.KubeManager.apply("opdk-istio", logger)
...
def main():
  ...
  os.environ["ENVOY_CONFIG"] = os.getenv("PWD") + "/opdk-envoy/envoy-config.yaml"
  cmd = [os.getenv("CLI_DIR", ".")+"/apigee-remote-service-cli", "samples", "create",
          "-c", "config.yaml", "--out", "opdk-envoy", "-t", "native"]
  process = subprocess.run(cmd, capture_output=True)
  if process.stderr != b'':
    logger.error(process.stderr.decode())
