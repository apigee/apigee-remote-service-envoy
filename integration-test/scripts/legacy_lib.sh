...
function undeployAPIProxies {
  echo -e "\nGet deployed revision of API Proxies ${1}..."
  REV=$(docker run curlimages/curl:7.72.0 --silent \
  ...
  if [[ ! -z $REV ]] ; then
    echo -e "\nUndeploying revision $REV of API Proxies ${1}..."
    STATUS_CODE=$(docker run curlimages/curl:7.72.0 -X DELETE --silent -o /dev/stderr -w "%{http_code}" \
      ${MGMTURL}/v1/organizations/${ORG}/environments/${ENV}/apis/${1}/revisions/${REV}/deployments \
      -u $USER:$PASSWORD)
    if [[ $STATUS_CODE -ge 299 ]] ; then
      echo -e "\nError undeploying API Proxies ${1}: $STATUS_CODE"
    fi
  fi
}
